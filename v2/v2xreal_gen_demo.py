...
def v2x_real_gen(transformation="insert", select_number=10, rq_name="demo", is_vis=True):
    """
    V2X-Real 数据集变换样例：从每个场景中选取 select_number 个数据帧执行 transformation 变换操作
    """
    dataset_config = DatasetConfig(dataset_name="v2x-real_dataset", rq_name=rq_name)
    ...
    for scene, bg_list in dataset_config.scenes_data_dict.items():
        ...
        for bg_index in selected_ids:
            ...
            if transformation == "insert":
                success_flag = op.vehicle_insert(ego_dataset, coop_dataset, is_vis=is_vis)
            elif transformation == "delete":
                success_flag = op.vehicle_delete(ego_dataset, coop_dataset, car_id, is_vis=is_vis)
            elif transformation == "translation":
                success_flag = op.vehicle_translation(ego_dataset, coop_dataset, car_id, is_vis=is_vis)
            elif transformation == "scaling":
                success_flag = op.vehicle_scaling(ego_dataset, coop_dataset, car_id, is_vis=is_vis)
            else:
                success_flag = op.vehicle_rotation(ego_dataset, coop_dataset, car_id, is_vis=is_vis)
