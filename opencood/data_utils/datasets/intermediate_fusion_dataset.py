...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            # TODO: add occlusion rate and distance for each object_id (intermediate_fusion_dataset)
            v2x_gen_dict = get_v2x_gen_dict(selected_cav_base)
            if cav_id == '0':
                processed_data_dict['ego'].update({'v2x_gen': v2x_gen_dict})
            else:
                processed_data_dict['ego'].update({'v2x_gen_cp': v2x_gen_dict})
    ...
    def collate_batch_test(self, batch):
        ...
        output_dict['ego'].update({'v2x_gen': batch[0]['ego']['v2x_gen']})

        if "v2x_gen_cp" in batch[0]['ego']:
            output_dict['ego'].update({'v2x_gen_cp': batch[0]['ego']['v2x_gen_cp']})
        else:
            output_dict['ego'].update({'v2x_gen_cp':{}})
