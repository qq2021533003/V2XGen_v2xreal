...
class BevPostprocessor(BasePostprocessor):
    ...
    def post_process(self, data_dict, output_dict):
        ...
        Step1: convert each cav's output to bounding box format 将每辆网联车辆 cav 的输出转换为边界框
        Step2: project the bounding boxes to ego space. 将边界框投影至自车坐标系下
        Step:3 NMS

        NOTE：V2X 多车协同 2D 目标检测后处理
            - 将模型输出结果转换成统一在自车坐标系下的 2D 框
        ...
        # TODO：看看为什么新插入车辆难以被预测到
        inserted_ids = data_dict['ego']['inserted_ids']
